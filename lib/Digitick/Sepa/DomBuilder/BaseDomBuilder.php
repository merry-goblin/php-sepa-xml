...
abstract class BaseDomBuilder implements DomBuilderInterface
{
     ...
     */
    public function __construct($painFormat, $withSchemaLocation = false)
    {
    ...
    public function visitGroupHeader(GroupHeader $groupHeader)
    {
        ...
        $creationDateTime = $this->createElement(
            'CreDtTm',
            substr($groupHeader->getCreationDateTime()->format($groupHeader->getCreationDateTimeFormat()),0,-1) // FXB : delete the "Z"
        );
		...
		$groupHeaderTag->appendChild($this->createElement('Grpg', $groupHeader->getGrpg()));
