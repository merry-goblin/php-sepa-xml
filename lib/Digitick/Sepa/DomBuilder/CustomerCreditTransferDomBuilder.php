...
class CustomerCreditTransferDomBuilder extends BaseDomBuilder
{
     ...
     */
    function __construct($painFormat = 'pain.001.002.02')
    {
    ...
    {
        $this->currentTransfer = $this->doc->createElement('pain.001.001.02'); //MODIF FXB
    ...
    public function visitPaymentInformation(PaymentInformation $paymentInformation)
    {
	   ...
	   * */
		...
		$PstlAdr->appendChild($this->createElement('AdrLine', $paymentInformation->getOriginAddressOne()));
		$PstlAdr->appendChild($this->createElement('AdrLine', $paymentInformation->getOriginAddressTwo()));
		$PstlAdr->appendChild($this->createElement('Ctry', $paymentInformation->getOriginCountry()));
    ...
    /**
     * Add the specific OrgId element for the format 'pain.001.001.02'
     *
    ...
    public function visitGroupHeader(GroupHeader $groupHeader)
    {
        ...
        if ($groupHeader->getInitiatingPartyId() !== null && $this->painFormat === 'pain.001.001.02') {
            $newId = $this->createElement('Id');
            $orgId = $this->createElement('OrgId');
            $othr  = $this->createElement('PrtryId');
